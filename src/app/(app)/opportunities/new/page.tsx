      ...
      <Card>
        {customers.length === 0 ? (
          ...
          <form action={createOpportunity} className="space-y-3">
            <Field label="Customer"><Select name="customer_id" defaultValue={preselect} blank="Choose…" required options={customers.map((c) => ({ key: c.id, label: c.full_name }))} /></Field>
            <Field label="Title" hint="e.g. Demir family – sea-view apartment"><input name="title" className="input" required /></Field>
            <Field label="Investment track"><Select name="track" defaultValue="home" options={[{ key: "home", label: "Apartment / home" }, { key: "land", label: "Plot / land" }]} /></Field>
            <Field label="Target timing (customer's words)"><input name="target_timing" className="input" placeholder="Wants to decide before year end" /></Field>
